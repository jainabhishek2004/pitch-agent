...
export async function renderPdfBuffer(html) {
  ...
  // Serverless (Vercel): the chromium binary can't be bundled, so @sparticuz/chromium-min
  // downloads a matching chromium "pack" at runtime (cached in /tmp on warm invocations).
  // Pack version MUST match puppeteer-core's Chrome version (both 131 here).
