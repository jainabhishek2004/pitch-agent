...
import { productById, painsForType, agents as ALL_AGENTS, brand } from "./knowledge.mjs";
...
export async function generatePitch({ cardText = "", website = "" }) {
  ...
  const pitch = buildPitch({ clientName, website, cardText, research, cls });
...
export function buildPitch({ clientName, website, cardText = "", research, cls }) {
  const primaryProduct = productById(cls.rule.primary);
  const secondary = cls.rule.secondary.map(productById).filter(Boolean);
  let expansion = cls.rule.expansion.map(productById).filter(Boolean);
  if (cls.erpDetected && !expansion.find((p) => p.id === "erp-integration-layer")) {
    expansion = [...expansion, productById("erp-integration-layer")].filter(Boolean);
  }
  const aiLayer = cls.rule.aiLayer
    .map((name) => ALL_AGENTS.find((a) => a.name === name))
    .filter(Boolean);
  ...
  for (const type of [cls.primary, ...cls.detected]) {
    for (const p of painsForType(type)) {
  ...
  const features = pains.slice(0, 5).map((p) => {
    const src = p._src;
    const prod = productById(src.products[0]);
